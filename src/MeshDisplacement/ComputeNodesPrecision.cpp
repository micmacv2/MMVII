...
namespace MMVII
{
        ...
        {
                const bool aRealCoordPointInsideDispX = mDImDispX->InsideInterpolator(*mInterpol, aRealCoordPoint, 0);
                const bool aRealCoordPointInsideDispY = mDImDispY->InsideInterpolator(*mInterpol, aRealCoordPoint, 0);
                if (aRealCoordPointInsideDispX && aRealCoordPointInsideDispY)
                        return tPt2dr(mDImDispX->GetValueInterpol(*mInterpol, aRealCoordPoint), mDImDispY->GetValueInterpol(*mInterpol, aRealCoordPoint));
                else if (aRealCoordPointInsideDispX && !aRealCoordPointInsideDispY)
                        return tPt2dr(mDImDispX->GetValueInterpol(*mInterpol, aRealCoordPoint), mDImDispY->GetV(aIntCoordPoint));
                else if (!aRealCoordPointInsideDispX && aRealCoordPointInsideDispY)
                        return tPt2dr(mDImDispX->GetV(aIntCoordPoint), mDImDispY->GetValueInterpol(*mInterpol, aRealCoordPoint));
                else if (!aRealCoordPointInsideDispX && !aRealCoordPointInsideDispY)
                        return tPt2dr(mDImDispX->GetV(aIntCoordPoint), mDImDispY->GetV(aIntCoordPoint));
                else
        ...
        void cAppli_ComputeNodesPrecision::LoopOverTrianglesAndGetDiffDispMaps()
        {
                ...
                for (size_t aTr = 0; aTr < mDelTri.NbFace(); aTr++)
                {
                        ...
                        const tPt2dr aTrPointA = CheckReturnOfInterpolatorValue(aRealCoordPointA, aIntCoordPointA); // Get value value from real or int coordinates 1st point
                        const tPt2dr aTrPointB = CheckReturnOfInterpolatorValue(aRealCoordPointB, aIntCoordPointB); // Get value value from real or int coordinates 2nd point
                        const tPt2dr aTrPointC = CheckReturnOfInterpolatorValue(aRealCoordPointC, aIntCoordPointC); // Get value value from real or int coordinates 3rd point
        ...
        int cAppli_ComputeNodesPrecision::Exe()
        {
                ...
                std::unique_ptr<const cDiffInterpolator1D> mInterpol = std::unique_ptr<const cDiffInterpolator1D>(cDiffInterpolator1D::AllocFromNames(mInterpolArgs));
