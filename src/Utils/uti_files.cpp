...
template<class Type> std::unique_ptr<Type>  NNfs(const std::string & aNameFile,const std::string & aMode,const std::string & aMes)
{
    ...
    if (!aRes->good())
    {
       MMVII_INTERNAL_ASSERT_user(false,"Cannot open file : "  + aNameFile + " ,Mode=" + aMode+  " ,context=" + aMes);
    }
   ...
   mName (aName)
{
    if (!mOfs.good())
    {
       MMVII_INTERNAL_ASSERT_user(false,"Cannot open file : "  + mName + " in mode write");
    }
}
...
std::ofstream & cMMVII_Ofs::Ofs()
{
   ...
   if (!mOfs.good())
   {
       MMVII_INTERNAL_ASSERT_user(false,"Bad file for "+mName);
   }
#endif
   return mOfs;
}
   ...
   mName (aName)
{
    if (!mIfs.good())
    {
       MMVII_INTERNAL_ASSERT_user(false,"Cannot open file : "  + mName + " in mode read");
    }
}
