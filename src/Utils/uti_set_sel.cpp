...
tNameSet SetNameFromString(const std::string & aName,bool AllowPat)
{
   ...
   if (ExistFile(aName))
      MMVII_INTERNAL_ASSERT_user(false,"Set from string, file has bad format for :" +aName);
...
tNameRel  RelNameFromFile (const std::string& aNameFile)
{
   ...
   if (ExistFile(aNameFile))
      MMVII_INTERNAL_ASSERT_user(false,"Set from string, file has bad format for :" +aNameFile);
