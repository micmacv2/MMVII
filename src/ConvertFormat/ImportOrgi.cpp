...
int cAppli_ImportOrgi::Exe()
{
    ...
    {
         cReadFilesStruct aFileModel(mOrgiFolder+mNameFileModel,"IES",0,-1,-1);
         aFileModel.Read();

         for (int aK=0 ; aK< aFileModel.NbRead() ; aK++)
         {
             mMapNum2Im[aFileModel.VInts().at(aK).at(0)] = aFileModel.VNameIm().at(aK);
         }
    }

    //  Read the 3D measures of GCP
    {
         cReadFilesStruct aFileAmers(mOrgiFolder+mNameFileAmer,"NXYZ",0,-1,-1);
         aFileAmers.Read();
         for (int aK=0 ; aK<aFileAmers.NbRead() ; aK++)
         {
             const std::string & aNamePt = aFileAmers.VNamePt().at(aK);
             const cPt3dr &  aPt         = aFileAmers.VXYZ().at(aK);
    ...
    {
         cReadFilesStruct aFileAppuis(mOrgiFolder+mNameFileAppui,"EEXY",0,-1,-1);
         aFileAppuis.Read();
         for (int aK=0 ; aK<aFileAppuis.NbRead() ; aK++)
         {
                 StdOut() << " " << aFileAppuis.VXYZ().at(aK) << "\n";
         }
    }
