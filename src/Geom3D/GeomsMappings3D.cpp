...
template <class Type>  cPtxd<Type,3> cIsometry3D<Type>::DiffInOut(const tPt & aPIn,const tPt & aPOut) const {return Value(aPIn)-aPOut; }
...
template <class Type> cSimilitud3D<Type>::cSimilitud3D() :
       cSimilitud3D<Type>((Type)1.0,cPtxd<Type,3>(0,0,0), cRotation3D<Type>())
{
}
...
template <class Type>  cSimilitud3D<Type> cSimilitud3D<Type>::FromMinimalSamples(const tTabMin& aTabIn,const tTabMin& aTabOut)
{
  tTri  aTriIn  (aTabIn[0], aTabIn[1], aTabIn[2]);
  tTri  aTriOut(aTabOut[0],aTabOut[1],aTabOut[2]);

  return FromTriInAndOut(0,aTriIn,0,aTriOut);
}

template <class Type>  cPtxd<Type,3> cSimilitud3D<Type>::DiffInOut(const tPt & aPIn,const tPt & aPOut) const {return Value(aPIn)-aPOut; }
...
template <class Type> void  cSimilitud3D<Type>::ToEqParam(tPt & aRHS,std::vector<cDenseVect<Type>>& aVV,const tPt &In,const tPt & Out)
{
    ...
    for (int aK=0; aK<3 ; aK++)
        aVV.at(aK)(SimInd_Scale) =In[aK];
}
template <class Type>  cSimilitud3D<Type>   cSimilitud3D<Type>::FromParam(const cDenseVect<Type> & aV)
{
